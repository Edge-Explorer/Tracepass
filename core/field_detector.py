...
- Honeypot pre-filtering (removes hidden / offscreen traps)
...
class FieldDetector:
    ...
    def is_honeypot(cls, element: Any) -> bool:
        ...
        return any(trap in class_name for trap in ("honeypot", "hidden-field", "visually-hidden"))

    @classmethod
    def detect_username(cls, response: Any) -> tuple[Any | None, str | None]:
        """Discovery waterfall for username / email field."""
        # Query standard inputs and modern web component inputs
        candidates = response.css("input, faceplate-text-input, reddit-input")

        # Filter out honeypots and non-textual inputs
        valid_candidates = [
            el
        ...
        for el in valid_candidates:
            ...
            if "username" in autocomplete or "email" in autocomplete:
                return el, el.generate_css_selector
        ...
        # Tier 5: Fallback to the first visible text input
        first_el = valid_candidates[0]
        return first_el, first_el.generate_css_selector
    ...
    def detect_password(cls, response: Any) -> tuple[Any | None, str | None]:
        ...
        # Return the first password field found
        pw_el = valid_candidates[0]
        return pw_el, pw_el.generate_css_selector

    @classmethod
    def detect_submit(cls, response: Any) -> tuple[Any | None, str | None]:
    ...
    def detect_fields(cls, response: Any) -> dict[str, str | None]:
        ...
        _, username_sel = cls.detect_username(response)
        _, password_sel = cls.detect_password(response)
        _, submit_sel = cls.detect_submit(response)

        # Check for multi-step 'Next' button if only username is visible
        next_sel = None
        if username_sel and not password_sel:
            # If no password is present, the submit button is acting as the 'Next' step
            next_sel = submit_sel
            submit_sel = None

        # Check for modal trigger if no login fields are in DOM
        modal_trigger_sel = None
        if not username_sel and not password_sel:
            modal_triggers = response.css("button, a")
            for trigger in modal_triggers:
                if not cls.is_honeypot(trigger):
                    ...
                    if aria_haspopup == "dialog" or cls.MODAL_TRIGGER_RE.search(text):
                        modal_trigger_sel = trigger.generate_css_selector
                        break
